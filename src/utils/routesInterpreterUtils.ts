import type { LiFiStep, LiFiStepExtended, Route, RouteExtended } from '@lifi/sdk';
...
export const handleTransactionDetails = (route: RouteExtended, customData: object) => {
...
export const filterRoutesObject = (
  ...
  route.steps.forEach((step: LiFiStep | LiFiStepExtended, index) => {
    stepData = {
      ...
      [TrackingEventParameter.TransactionId]: step.includedSteps[step.includedSteps.length - 1].id,
    ...
    const detailInformation = isExtendedStep && step.execution ? step.execution : step.estimate;
...
export const getAction = (route?: RouteExtended) => {
  ...
  for (const step of route.steps) {
    return step.execution?.process.find(
      (process) => process.status === 'PENDING' || process.status === 'ACTION_REQUIRED',
    );
  }
