...
function getImageUrl(
  ...
  const imageUrl =
    defaultMode === 'light' ? imageLight?.data?.attributes.url : imageDark?.data?.attributes.url;

  return imageUrl ? new URL(imageUrl, baseStrapiUrl) : null;
}

export function getAvailableThemeModes(theme?: PartnerThemesAttributes): string[] {
  const result: string[] = [];
...
export function formatConfig(theme?: PartnerThemesAttributes): Partial<PartnerThemeConfig> {
  if (!theme) {
    return {
      uid: 'default',
