...
export function useUserTracking() {
  ...
  // const false = useMediaQuery( theme.breakpoints.up('md'));
  const sessionId = useSession();

  const { trackEvent: jumperTrackEvent, trackTransaction: jumperTrackTransaction } =
    useJumperTracking();
  ...
  const trackEvent = useCallback(
    ...
    [account?.address, account?.isConnected, false, jumperTrackEvent, sessionId],
  );
