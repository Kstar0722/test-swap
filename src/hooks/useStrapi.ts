...
export const useStrapi = <T>({
    ...
    if (filterTag) {
      if (typeof filterTag === 'string') {
        ...
        filterTag.forEach((el, index) => {
          apiUrl.searchParams.set(`filters[$and][0][$or][${index}][tags][id][$eq]`, `${el}`);
        });
      }
    }
  ...
  if (contentType === STRAPI_FEATURE_CARDS) {
    ...
    apiUrl.searchParams.set('filters[$or][0][campaignStart][$lte]', currentDate);
  ...
  if (contentType === STRAPI_JUMPER_USERS) {
    ...
    apiUrl.searchParams.set('populate[feature_cards][populate][0]', 'BackgroundImageLight');
    apiUrl.searchParams.set('populate[feature_cards][populate][1]', 'BackgroundImageDark');
    apiUrl.searchParams.set('populate[feature_cards][populate][2]', 'featureCardsExclusions');
