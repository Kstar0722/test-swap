...
import { type Connector, useConnect, useAccount as useWagmiAccount } from 'wagmi';

import { defaultCoinbaseConfig } from '@/configs/coinbase';
import { defaultMetaMaskConfig } from '@/configs/metaMask';
import { defaultWalletConnectConfig } from '@/configs/walletConnect';
...
const combineWalletLists = (
  ...
  const combined: CombinedWallet[] = evmConnectorList.map((evm) => {
    const matchedSvm = svmWalletList?.find((svm) => svm.adapter.name === evm.name);
    return {
      evm,
      svm: matchedSvm,
    };
...
export const useCombinedWallets = () => {
  ...
  const [combinedInstalledWallets, setCombinedInstalledWallets] = useState<CombinedWallet[]>([]);

  const [combinedNotDetectedWallets, setCombinedNotDetectedWallets] = useState<CombinedWallet[]>(
    [],
  );

  // combine installed wallets
  useEffect(() => {
    const evmConnectors: (CreateConnectorFnExtended | Connector)[] = Array.from(connectors);
    if (!connectors.some((connector) => connector.id.toLowerCase().includes('walletconnect'))) {
      evmConnectors.unshift(createWalletConnectConnector(defaultWalletConnectConfig));
    }
    if (
      !connectors.some((connector) => connector.id.toLowerCase().includes('coinbase')) &&
      !isWalletInstalled('coinbase')
    ) {
      evmConnectors.unshift(createCoinbaseConnector(defaultCoinbaseConfig));
    }
    if (
      !connectors.some((connector) => connector.id.toLowerCase().includes('metamask')) &&
    ...
    const evmNotDetected = evmConnectors.filter((connector) => !isWalletInstalled(connector.id));

    const svmNotDetected = solanaWallets?.filter(
      (connector) => connector.adapter.readyState !== WalletReadyState.Installed,
    );
    const notDetectedWallets = combineWalletLists(evmNotDetected, svmNotDetected);
  ...
  useEffect(() => {
    const multiSigInstalled = async () => {
      const safeWallet = connectors.find((connector) => connector.name === 'Safe');
