import type { MultisigConfig, MultisigTransaction, MultisigTxDetails } from '@lifi/sdk';
...
export const useMultisig = () => {
  ...
  const checkMultisigEnvironment = async () => {
    ...
    const isIframeEnvironment = typeof window !== 'undefined' && window.parent !== window;
  ...
  const handleMultiSigTransactionDetails = async (
    ...
    if (isSafeStatusPending) {
      ...
      return await handleMultiSigTransactionDetails(txHash, chainId, updateIntermediateStatus);
    }

    if (
      [safeTransactionDetails.txStatus, safeApiTransactionDetails.txStatus].includes(
        TransactionStatus.SUCCESS,
      )
    ) {
      return {
  ...
  const shouldOpenMultisigSignatureModal = (route: Route) => {
    const isRouteDone = route.steps.every((step) => (step as any).execution?.status === 'DONE');

    const isRouteFailed = route.steps.some((step) => (step as any).execution?.status === 'FAILED');

    const multisigRouteStarted = route.steps.some((step) =>
      (step as any).execution?.process.find(
        (process: Process) => !!process.multisigTxHash && process.status === 'ACTION_REQUIRED',
      ),
    );
