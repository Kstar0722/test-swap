...
import { getConnectorIcon, isWalletInstalled } from '@lifi/wallet-management';
...
export const useWalletSelectContent = () => {
  ...
  const { combinedInstalledWallets, combinedNotDetectedWallets } = useCombinedWallets();
  ...
  const { setSnackbarState, closeAllMenus, setEcosystemSelectMenuState } = useMenuStore(
    (state) => state,
  );
  ...
  const availableWallets = useMemo(() => {
    ...
    if (isDesktopView) {
      allowedWallets = [...combinedInstalledWallets, ...combinedNotDetectedWallets];
    }
  ...
  const connectWallet = useCallback(
    ...
    [closeAllMenus, setCookie, setEcosystemSelectMenuState, connect, setSnackbarState, t],
  );
  ...
  const walletMenuItems = useMemo<MenuListItem[]>(() => {
    const handleWalletClick = async (combinedWallet: CombinedWallet) => {
      if (
        ...
        (combinedWallet.svm && combinedWallet.svm.adapter.readyState === WalletReadyState.Installed)
      ) {
      ...
      return {
        ...
        prefixIcon: (
          <Avatar
            ...
            src={getConnectorIcon((combinedWallet.evm as Connector) || combinedWallet.svm?.adapter)}
            alt={`${combinedWallet.evm?.id || combinedWallet.svm?.adapter.name}-wallet-logo`}
            sx={{
              height: 40,
              width: 40,
              objectFit: 'contain',
    ...
    return output;
  }, [availableWallets, connectWallet, closeAllMenus, setSnackbarState, t, theme, trackEvent]);
