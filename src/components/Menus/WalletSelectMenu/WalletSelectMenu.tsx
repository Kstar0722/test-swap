...
import { MenuHeaderAppBar, MenuHeaderAppWrapper } from '@/components/Menu/Menu.style';
...
export const WalletSelectMenu = ({ anchorEl }: MenuProps) => {
  ...
  const filteredWalletSelectMenuItems = walletSelectMenuItems.slice(0, NUMBER_OF_WALLETS_DISPLAYED);
  const { openWalletSelectMenu, setWalletSelectMenuState, openSubMenu, setSubMenuState } =
      ...
      {walletSelectMenuItems.length - NUMBER_OF_WALLETS_DISPLAYED > 0 &&
        openSubMenu === MenuKeysEnum.None && (
          <MenuItem
            ...
            showMoreIcon={false}
            onClick={() =>
              openSubMenu === MenuKeysEnum.None && setSubMenuState(MenuKeysEnum.WalletSelectMore)
            }
            styles={{
              ...menuItemStyles,
              gridColumn: 'span 3',
              ...
              '&:hover button': {
                backgroundColor: getContrastAlphaColor(theme, '36%'),
              },
            }}
            prefixIcon={
              <Typography
