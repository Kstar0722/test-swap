...
import { ButtonSecondary, ButtonTransparent } from 'src/components/Button2';
...
export const WalletCard = ({ account }: WalletCardProps) => {
    ...
    if (account.blockChainExplorerUrl) {
      trackEvent({
        ...
        data: {
          ...
          [TrackingEventParameter.PageloadURL]: account.blockChainExplorerUrl || '',
  ...
  return (
    <WalletCardContainer>
      <Stack direction={'row'} spacing={3} sx={{ margin: 'auto', flexGrow: 1 }}>
        <WalletCardBadge
          overlap="circular"
          className="badge"
          ...
          badgeContent={
            activeChain?.logoURI ? (
              <Avatar size="large" src={activeChain?.logoURI || ''} alt={'wallet-avatar'}>
                {/* {activeChain.name[0]} */}
              </Avatar>
            ) : (
        ...
        <WalletCardButtonContainer>
          ...
          >
            <Typography variant="bodySmallStrong">{walletDigest(account?.address)}</Typography>
          </ButtonTransparent>
          <ButtonTransparent
            size="medium"
            sx={{
