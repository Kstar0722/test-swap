...
export const MenuList = styled(MuiMenuList, {
  ...
  '& > :first-of-type': {
    marginTop: isOpenSubMenu || hasLabel || cardsLayout ? 'inherit' : theme.spacing(1.5),
  },
  '& > li:last-of-type': {
...
export interface MenuPaperProps extends Omit<PaperProps, 'isDarkMode' | 'isWide' | 'component'> {
  isMobile?: boolean;
  show: boolean;
  width?: string;
  component?: string;
...
export const MenuPaper = styled(Paper, {
  shouldForwardProp: (prop) => prop !== 'isMobile' && prop !== 'isWide' && prop !== 'show',
...
export const MenuHeaderAppWrapper = styled(ListItem)<ListItemProps>(({ theme }) => ({
  position: 'sticky',
  top: 0,
  alignItems: 'center',
...
export const MenuHeaderAppBar = styled(AppBar)<MenuHeaderAppBarProps>(({ theme }) => ({
